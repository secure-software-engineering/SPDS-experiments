 ...
 * <ul>
 * <li>to support exceptional control flow ... there may be several return sites
 * for each call site.
 * <li>it supports an optional merge operator, useful for non-IFDS problems and
 * widening.
 ...
 * 
 * @param <T>
 *          type of node in the supergraph
 * @param <P>
...
public class TabulationSolver<T, P, F> {
  ...
  static protected final boolean verbose = true
      && ("true".equals(System.getProperty("com.ibm.wala.fixedpoint.impl.verbose")) ? true : false);
  ...
  /**
   * the set of all {@link PathEdge}s that were used as seeds during the
   * tabulation, grouped by procedure.
   */
  ...
  private final Set<P> setsOfVisitedProcedures = HashSetFactory.make();
  ...
  /**
   * the path edge currently being processed in the main loop of
   * {@link #forwardTabulateSLRPs()}; <code>null</code> if
   * {@link #forwardTabulateSLRPs()} is not currently running. Note that if we
   * are applying a summary edge in {@link #processExit(PathEdge)}, curPathEdge
   * is modified to be the path edge terminating at the call node in the caller,
   * to match the behavior in {@link #processCall(PathEdge)}.
   */
  private PathEdge<T> curPathEdge;

  /**
   * the summary edge currently being applied in {@link #processCall(PathEdge)}
   * or {@link #processExit(PathEdge)}, or <code>null</code> if summary edges
  ...
  private long maxTimeBudgetInMs = 30000;

  private int propagationCountIncludingZero;
  ...
  /**
   * Subclasses can override this to plug in a different worklist
   * implementation.
   */
  ...
  private void propagateToReturnSites(final PathEdge<T> edge, final T c, final IntSet D4) {
    ...
    for (Iterator<? extends T> retSites = supergraph.getReturnSites(c, supergraph.getProcOf(edge.target)); retSites.hasNext();) {
      ...
      // so, we'll filter the logic by checking that we only process reachable
      // return sites.
   ...
   * @param entries
   *          entry nodes in the caller
   * @param retSite
   *          the return site
   * @param d4
   *          a fact s.t. <c, d4> -> <callee, d2> was recorded as call flow and
   *          <callee, d2> is the source of the summary edge being applied
   * @param D5
   *          facts to propagate to return site
   */
          ...
          for (int i = 0; i < entries.length; i++) {
            ...
            if (D3 != null) {
              D3.foreach(new IntSetAction() {
                @Override
                public void act(int d3) {
                  // set curPathEdge to be consistent with its setting in
                  // processCall() when applying a summary edge
   ...
   * 
   * @param edge
   *          the path edge being processed
   * @param callNodeNum
   *          the number of the call node in the supergraph
   * @param allReturnSites
   *          a set collecting return sites for the call. This set is mutated
   *          with the return sites for this callee.
   * @param calleeEntry
  ...
  protected void processParticularCallee(final PathEdge<T> edge, final int callNodeNum, Collection<T> allReturnSites,
    ...
    final Collection<T> returnSitesForCallee = Iterator2Collection
        .toSet(supergraph.getReturnSites(edge.target, supergraph.getProcOf(calleeEntry)));
  ...
  protected boolean propagate(T s_p, int i, T n, int j) {
    if(isTimeout()){
      throw new IFDSSolverTimeoutException();
    }
    ...
    if (!pLocal.contains(i, number, j)) {
      ...
      if(i != 0){
        P procOf = supergraph.getProcOf(s_p);
        setsOfVisitedProcedures.add(supergraph.getProcOf(s_p));
        if(procOf instanceof CGNode){
          CGNode cgNode = (CGNode) procOf;
          if(!cgNode.getMethod().isNative())
            propagationCount++;
        }
      }
      propagationCountIncludingZero++;
      return true;
    }
    return false;
  }
  ...
  private boolean isTimeout() {
    return (System.currentTimeMillis() - startTime > maxTimeBudgetInMs);
  }
  ...
  public class Result implements TabulationResult<T, P, F> {
    ...
    /*
     * @see
     * com.ibm.wala.dataflow.IFDS.TabulationResult#getSupergraphNodesReached()
     */
    @Override
  ...
  public long getPropagationCountIncludingZero() {
    return propagationCountIncludingZero;
  }
  public Set<P> getVisitedProcedures() {
    return setsOfVisitedProcedures;
  }
}
