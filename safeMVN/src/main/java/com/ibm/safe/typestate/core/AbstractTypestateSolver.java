...
import com.ibm.wala.dataflow.IFDS.IFDSSolverTimeoutException;
...
public abstract class AbstractTypestateSolver extends AbstractWholeProgramSolver {
	...
	/**
	 * An object that holds information on statements and instances that are
	...
	private long propagationCounts;
	private long analysisTimes;
	private Set<CGNode> visitedMethods = new HashSet<CGNode>();
	private Set<CGNode> totalVisitedMethods = new HashSet<CGNode>();

	private HashSet<Pair<String, String>> seeds;

	private String seedDeclaringClass;
	private String seedDeclaringMethodName;

	private long propagationCountsInclZero;

	private long totalPropagationCount;
	 ...
	 */
	public AbstractTypestateSolver(CallGraph cg, PointerAnalysis pointerAnalysis, ITypeStateDFA dfa,
	...
	/**
	 * construct an abstract typestate solver with a null trace reporter TODO:
	 * the mining code (trace reporter) will be eventually refactored out of
	 * here
	 * 
	 * @param cg
	 * @param pointerAnalysis
	...
	public AbstractTypestateSolver(CallGraph cg, PointerAnalysis pointerAnalysis, TypeStateProperty property,
			TypeStateOptions options, ILiveObjectAnalysis live, BenignOracle ora, TypeStateMetrics metrics,
			IReporter reporter, IMergeFunctionFactory mergeFactory) {
	 ...
	 */
	protected Collection<CGNode> computeNodesThatMatter(OrdinalSet<InstanceKey> instances)
	...
	/**
	 * @param instances
	 * @return an ordinal set of these instances, indexed by the pointer
	 *         analysis instance key mapping
	 */
	 ...
	 * and update the property accordingly. Not the most beautiful way to do
	 * this.
	 * 
	...
	protected boolean initializeProperty() throws PropertiesException {
		...
		if (getOptions().shouldMineDFA()) {
			...
			for (Iterator<String> it = rule.getTypes().iterator(); it.hasNext();) {
				...
				IClass pType = TypeStatePropertyContext.getPropertyTrackedType(getCallGraph().getClassHierarchy(),
						typeDef);
				if (pType != null) {
	 ...
	 * 
	 * @param e
	 * @return true iff some non-self transition in the DFA on event e goes to
	 *         an accepting state
	 */
	 ...
	 */
	protected static Predicate<CGNode> makeLiveNodeFilter(final OrdinalSet<InstanceKey> instances,
			final ILiveObjectAnalysis live) {
	...
	protected Collection<InstanceKey> computeTrackedInstancesByType() {
		...
		for (Iterator<InstanceKey> it = getPointerAnalysis().getInstanceKeys().iterator(); it.hasNext();) {
			...
			if (TypeStatePropertyContext.isTrackedType(getCallGraph().getClassHierarchy(), getDFA().getTypes(),
					instanceType)) {
	...
	protected Collection<InstanceKey> computeRelevantReceiversByDFA() throws PropertiesException {
		...
		/**
		 * For each instance, check that the instance-specific DFA includes a
		...
		for (Iterator<Map.Entry<InstanceKey, Set<IEvent>>> it = m.entrySet().iterator(); it.hasNext();) {
			...
			DFSPathFinder<Object> search = new DFSPathFinder<Object>(dfa, start, acceptFilter) {
				...
				/**
				 * get the out edges of a given node, tracing only edges
				 * labelled with appropriate events
				 * 
				 * @param n
	 ...
	 * @param instance
	 *            an instance
	 * @return true if there is some caller of an event e \in events such that
	 *         the event may transition to an accepting state when invoked on
	 *         the instance, and the caller is from the application class loader
	 */
		...
		for (Iterator<CGNode> it = getCallGraph().iterator(); it.hasNext();) {
			...
			if (getDFA().receives(n.getMethod())) {
				...
				if (events.contains(e) && eventTransitionsToAccept(e)) {
					Collection<CGNode> callers = computeRelevantCallers(Collections.singleton(n),
	...
	/**
	 * set up a mapping of instance keys to events that the instance key flows
	 * to
	 * 
	 * If NO_LIBRARY_ERRORS, ignore events that can only cause errors from
	 * library code.
	 * 
	...
	/**
	 * The returned result does not include benign statements, nor statements
	 * for which we already have a finding.
	 * 
	 * @param targets
	 * @param receivers
	 ...
	 */
	@SuppressWarnings("unused")
	protected TypeStateResult solveForInstances(Collection<InstanceKey> instances, AnalysisCache ac)
			throws WalaException, PropertiesException, SetUpException, CancelException {
		...
		propagationCounts = 0;
		visitedMethods = new HashSet<CGNode>();
		analysisTimes = 0;
		...
		if (relevantNodes.size() == 0) {
			...
			TabulationSolver<BasicBlockInContext<IExplodedBasicBlock>, CGNode, Factoid> solver = TabulationSolver
					.make(p);
			...
			propagationCounts = solver.getPropagationCount();
			propagationCountsInclZero = solver.getPropagationCountIncludingZero();
			totalPropagationCount += solver.getPropagationCount();
			if(solver.getPropagationCount() != 0)
				totalSeeds++;
			visitedMethods = new HashSet<CGNode>(solver.getVisitedProcedures());
			totalVisitedMethods.addAll(visitedMethods);
			System.err.println("Took : " + (after - before) + " ms ");
			analysisTimes = (after - before);
			totalAnalysisTime += analysisTimes;
			System.out.println("PropagationCount:" + solver.getPropagationCount() + "  " + instances);
			if (DEBUG_LEVEL > 0) {
	...
	private int computeReachableMethods() {
		...
		Set<MethodReference> methods = new HashSet<MethodReference>();
		for (CGNode n : reachableNodes) {
			if(!n.getMethod().isNative())
				methods.add(n.getMethod().getReference());
		}
//    	FileWriter fileWriter;
//		try {
//			fileWriter = new FileWriter("-fink-reachablemethods.csv");
//	    	fileWriter.write("MethodName;DeclaringClass;Signature\n");
//
//			for(MethodReference m : methods){
//	    		fileWriter.write(m.getName()+";"+m.getDeclaringClass()+";"+m.toString()+";\n");
//			}
//		} catch (IOException e) {
...
//		}
	 ...
	 * 			@throws
	 */
	public ISolverResult perform() throws WalaException, SolverTimeoutException,
			PropertiesException, MaxFindingsException, SetUpException, CancelException {
		...
		System.err.println("reachableMethods: " + computeReachableMethods());
		...
		try {
			...
			if (hasPropertyType) {
				...
				if (getOptions().shouldCollectStatistics() && !getOptions().shouldMineDFA()) {
					getMetrics().setNumberOfCandidateStatements(getPropertyName(),
							countCandidateStatements(computeTrackedInstancesByType()));
				}
...
//							}
					...
					} catch(IFDSSolverTimeoutException e){
						timedout = true;
						System.out.println("Seed timeout");
					}
					if(hasError){
						errorCount ++;
					}
		...
		if(isTestsuite()){
			...
			if (output != null && !output.equals("")) {
				...
				try {
					...
					writer.write(String.format("%s;%s;%s;%s;%s;%s;%s;%s;\n", System.getProperty("method"), totalPropagationCount, totalVisitedMethods.size(),  errorCount,expected, totalSeeds,falsePositives,falseNegatives));
					writer.close();
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
	...
	private int computeVisitedMethods() {
		Set<IMethod> methods = new HashSet<IMethod>();
		for(CGNode nodes : visitedMethods){
			if(!nodes.getMethod().isNative())
				methods.add(nodes.getMethod());
		}
		return methods.size();
	}
	...
	/**
	 * In order to allow strong updates, we'd better not slice selectively at
	 * call sites that might call an event, because the flow functions treat
	 * sliced callees as identity flow; hence we'd lose strong updates by
	 * over-aggressive slicing there.
	 * 
	 * @param slice
	 *            Collection<CGNode>
	 * @param eventNodes
		...
		for (Iterator<CGNode> it = eventNodes.iterator(); it.hasNext();) {
			...
			if (slice.contains(event)) {
				for (Iterator<? extends CGNode> it2 = getCallGraph().getPredNodes(event); it2.hasNext();) {
					CGNode caller = it2.next();
					for (Iterator<CallSiteReference> it3 = getCallGraph().getPossibleSites(caller, event); it3
							.hasNext();) {
						CallSiteReference site = it3.next();
	...
	protected abstract TypeStateProblem createTypeStateProblem(ICFGSupergraph supergraph,
				...
				if (property.eventTransitionsToAccept(e)) {
					for (Iterator<? extends CGNode> it2 = getCallGraph().getPredNodes(n); it2.hasNext();) {
						...
						if (!NO_LIBRARY_ERRORS || isApplicationNode(caller)) {
							IR ir = caller.getIR();
							sites: for (Iterator<CallSiteReference> it3 = getCallGraph().getPossibleSites(caller,
									n); it3.hasNext();) {
								CallSiteReference site = it3.next();
								SSAAbstractInvokeInstruction[] calls = ir.getCalls(site);
								for (int i = 0; i < calls.length; i++) {
									int recv = calls[i].getReceiver();
									PointerKey pk = getPointerAnalysis().getHeapModel().getPointerKeyForLocal(caller,
											recv);
									Collection<InstanceKey> c = OrdinalSet
											.toCollection(getPointerAnalysis().getPointsToSet(pk));
