...
public final class StructuralSolverRunner extends AbstractSolverRunner {
  ...
  public void run(final IResultObserver observer) throws SafeException, SetUpException,
      CancelException {
    ...
    try {
      if (getModulesCodeKind(commonOptions) == CodeKind.J2SE) {
        IClassHierarchy cha = StructuralSolverRunner.createJ2SEClassHierarchy(getManager(), getPerformanceTracker());
        solvers = StructuralSolverRunner.createSolversStatic(cha, structuralOptions.getClassFilter(), structuralOptions,
            getReporter());
      } else {
        IClassHierarchy cha = StructuralSolverRunner.createJ2EEClassHierarchy(getManager(), getPerformanceTracker());
        solvers = StructuralSolverRunner.createSolversStatic(cha, structuralOptions.getClassFilter(), structuralOptions,
            getReporter());
      }
    } catch (IOException e) {
    ...
    runSolvers(solvers, observer, AnalysisKind.STRUCTURAL, Stages.STRUCTURAL, null);
  }
  ...
  public static IClassHierarchy createJ2SEClassHierarchy(final LightWeightOptions analysisOptions,
      final PerformanceTracker perfoTracker) throws SafeException {
    J2SEClassHierarchyEngine x = new J2SEClassHierarchyEngine(analysisOptions, perfoTracker);
  ...
  public static IClassHierarchy createJ2EEClassHierarchy(final LightWeightOptions analysisOptions,
      final PerformanceTracker perfoTracker) throws PropertiesException {
