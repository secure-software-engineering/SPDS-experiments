...
public abstract class AbstractSafeController {
   ...
   */
  public abstract void execute(final IRule[] rules, final IReporter reporter) throws SafeException,
  ...
  protected IRulesReader createRulesReader() throws PropertiesException {
    ...
    System.out.println(Props.RULES_DIRS +" "+rulesDirsProperty);
  ...
  protected void execute(final CompositeSolverRunner solverRunner, final PerformanceTracker perfoTracker,
      final IReporter originalReporter, final IReporter realReporter) throws SafeException,
      CancelException, IllegalArgumentException {
    try {
      ...
      solverRunner.run(resultObserver);
  ...
  protected static final String VERSION_PROPERTIES_FILE = "version.properties"; //$NON-NLS-1$
