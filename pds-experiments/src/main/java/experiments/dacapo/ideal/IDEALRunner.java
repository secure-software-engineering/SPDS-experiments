package experiments.dacapo.ideal;
...
public class IDEALRunner extends SootSceneSetupDacapo {
		...
		try {
			...
			return new IDEALAnalysis<TransitionFunction>(new IDEALAnalysisDefinition<TransitionFunction>() {
				...
				public Debugger<TransitionFunction> debugger(IDEALSeedSolver<TransitionFunction> solver) {
					...
					// return new IDEVizDebugger<>(new File("idealDebugger/" + solver.getSeed()),
					// icfg);
				}
	...
	private String asCSVLine(WeightedForwardQuery<TransitionFunction> key,
		...
		return String.format("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;\n", analysis, rule, key, seedStmt,
				seedMethod, seedClass, isInErrorState, isTimedout, analysisTime, propagationCount, visitedMethods,
				reachableMethods, containsCallLoop, containsFieldLoop, 0, usedMemory);
	}
