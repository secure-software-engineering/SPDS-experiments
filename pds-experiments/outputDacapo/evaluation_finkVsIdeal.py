    ...
    if "ideal" in fname and not "-ap" in fname and not "_vs_" in fname:
        ...
        for rowIDEAL in dataIDEAL:
            ...
            for rowFINK_UNIQUE in dataFINK_UNIQUE:
                if rowFINK_UNIQUE['SeedMethod'] == rowIDEAL['SeedMethod'] and int(rowFINK_UNIQUE['VisitedMethod']) != 0 and int(rowIDEAL['VisitedMethod']) != 0:
                    if not foundInFINK_UNIQUE:
                        isTimedoutIDEAL = rowIDEAL['Timedout'] == "true" or int(rowIDEAL['AnalysisTimes']) > MAX_ANALYSIS_TIME;
                        if isTimedoutIDEAL:
                            timeoutsIDEAL += 1
                        ...
                        if rowIDEAL['Is_In_Error'] == "true" or isTimedoutIDEAL:
                            errorsIDEAL += 1


                        isTimedoutFINK_UNIQUE = rowFINK_UNIQUE['Timedout'] == "true" or int(rowFINK_UNIQUE['AnalysisTimes']) > MAX_ANALYSIS_TIME;
                        if isTimedoutFINK_UNIQUE:
                        ...
                        if rowFINK_UNIQUE['Is_In_Error'] == "true" or isTimedoutFINK_UNIQUE:
                            errorsFINK_UNIQUE += 1
                    foundInFINK_UNIQUE = True
            if foundInFINK_UNIQUE:
                foundInFINK_APMUST = False
                for rowFINK_APMUST in dataFINK_APMUST:                    
                    if rowFINK_APMUST['SeedMethod'] == rowIDEAL['SeedMethod']:
                        if not foundInFINK_APMUST:
                            foundInFINK_APMUST = True
                            isTimedout = rowFINK_APMUST['Timedout'] == "true" or int(rowFINK_APMUST['AnalysisTimes']) > MAX_ANALYSIS_TIME;
                            if isTimedout:
                                timeoutsFINK_APMUST += 1
                                timesFINK_APMUST.append(MAX_ANALYSIS_TIME)
                            else:
                                timesFINK_APMUST.append(beautifyAnalysisTime(rowFINK_APMUST['AnalysisTimes']))

                            if rowFINK_APMUST['Is_In_Error'] == "true"  or isTimedout:
                                errorsFINK_APMUST += 1
               # for rowIDEAL in dataIDEAL:
                #    if (rowIDEAL['SeedMethod'] == rowIDEAL['SeedMethod'] and rowIDEAL['SeedStatement'] == rowIDEAL['SeedStatement']):
                 # #      timesIDEAL.append(beautifyAnalysisTime(rowIDEAL['AnalysisTimes']))
